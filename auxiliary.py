...
#Name of folder inside .config folder
CONFIG_BASE_DIR = "OCP"
...
class MessageBox():
    def __init__(self, parent_window, application):
        ...
        self.INFO_DIR = os.path.join(self.app.MyArgs.APP_DIR, 'info')
    ...
    def AboutBox(self):
        ...
        aboutdialog.set_program_name(self.app.MyArgs.localizedname)
        aboutdialog.set_version(self.app.MyArgs.name + ' v.' + self.app.MyArgs.version)
        ...
        if os.path.exists(ARTISTSFILE):
            with open(ARTISTSFILE, mode='rt', encoding='utf-8') as f:
                aboutdialog.set_artists(f.readlines())
    ...
    def Message(self, message, buttons='', boxtype = 'INFO', usethetitle = None, *args):
        ...
        dialog.set_title(self.app.MyArgs.localizedname)
...
class SectionConfig():
    ...
    def __init__(self, base_name, section):
        ...
        self.ok = False
        self.parser = None
        self.configfile = os.path.join(os.path.expanduser('~'), '.config', CONFIG_BASE_DIR, base_name + '.conf')
        create_empty_config(self.configfile,"SectionConfig")
...
class OptionConfig():
    ...
    Use class SectionConfig to read and write
    section 'General Options' from a conf file
    in user's home directory.
    All modifications are dumped immediately.
    ...
    def get(self, thekey, thedefault):
        with SectionConfig(self.base_name, self.general_section) as cf:
...
def create_empty_config(fullpath, test):
    thepath, base_name = os.path.split(fullpath)
    if not os.path.exists(thepath):
        try:
            pathlib.Path(thepath).mkdir(parents=True, exist_ok=True)
            pathlib.Path(fullpath).touch(exist_ok=True)
            #print("Successfully created config.")
        except Exception as e:
            print('Unable to open for writing or create %s (either folders or the file).' % fullpath, e)
            sys.exit(-3)
